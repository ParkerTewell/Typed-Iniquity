...
;   (check-equal? (run
;                   '(: map-add1 (-> (Listof Integer) (Listof Integer)))
;                  '(define (map-add1 xs)
;                     (if (empty? xs)
;                         '()
;                         (cons (add1 (car xs))
...
;                 'err)
   ...
   ;; Occurence typing examples
;   (check-equal? (run '(: add-1 (-> Any Integer))
;                      '(define (add-1 x) (if (int? x) (add1 x) 0))
;                      '(add-1 2))
;                 3)

  (check-equal? (run '(: f (-> (U String Integer) Integer))
                     '(define (f x) (if (int? x) (add1 x) (string-length x)))
                     '(f "hello"))
                5))   
